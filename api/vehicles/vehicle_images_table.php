...
$vehicle_images_migration = new Vehicle_Images_Migration();

if($_POST['action'] == "CREATE_TABLE"){
    $table = $vehicle_images_migration->create();
    if($table){
...
if($_POST['action'] == "FETCH_COLUMNS"){
    $results = $vehicle_images_migration->find_columns();
...
if($_POST['action'] == "DELETE_TABLE"){
    $table = $vehicle_images_migration->drop();
    if($table){
        $data['message'] = "success";
    }
